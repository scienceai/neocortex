...
describe('Layer: embedding', function() {
  ...
  let E = ndarray(new Float32Array(4*3), [4, 3]);
  ops.assign(E.pick(1, null), ndarray(new Float32Array([0.1, 0.2, 0.3]), [3]));
  ops.assign(E.pick(2, null), ndarray(new Float32Array([0.4, 0.5, 0.6]), [3]));
  ...
  it('should create zero-masked embedding matrix', (done) => {
    ...
    for (let i = 0; i < y.shape[0]; i++) {
      for (let j = 0; j < y.shape[1]; j++) {
        assert(almostEqual(y.get(i, j), E.get(i+1, j), almostEqual.FLT_EPSILON, almostEqual.FLT_EPSILON));
      }
    }
    done();
  });
